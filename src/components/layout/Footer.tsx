  ...
  return (
    <footer className="bg-brand-black py-14 text-white" id="contact">
      <Container>
            ...
            {footerColumns.map((column) => {
              ...
              return (
                <section key={column.title}>
                  <h2 className="font-mono text-sm font-bold uppercase text-brand-blue">{column.title}</h2>
                  <div className="mt-4 space-y-2 font-body text-sm leading-6 text-white/75">
                    {column.lines.map((line) => (
                      <p className="flex gap-2" key={line}>
                        {Icon ? <Icon aria-hidden className="mt-1 shrink-0 text-white/45" size={15} /> : null}
                        <span>{line}</span>
                      </p>
                    ))}
                  </div>
        ...
        <div className="mt-14 flex flex-col gap-4 border-t border-white/10 pt-8 font-mono text-sm uppercase tracking-[0.12em] text-white/45 sm:flex-row sm:items-center sm:justify-between">
